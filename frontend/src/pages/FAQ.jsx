      ...
      <section className="py-24 sm:py-32">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.6 }} className="mb-16">
            ...
            <h1 className="font-['Oswald'] text-5xl sm:text-6xl font-bold uppercase mt-4 text-white">FAQ</h1>
          </motion.div>

          <div className="space-y-12">
            {categories.map((cat, ci) => (
              <motion.div key={ci} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5 }}>
                <h2 className="font-['Oswald'] text-xl font-bold uppercase text-[#D4AF37] mb-4">{cat.title}</h2>
                <Accordion type="single" collapsible className="space-y-2">
                  {cat.items.map((item, i) => (
                    <AccordionItem key={i} value={`${ci}-${i}`} className="border border-white/5 bg-[#121212] px-6" data-testid={`faq-item-${ci}-${i}`}>
                      <AccordionTrigger className="text-left text-sm font-medium text-white hover:text-[#D4AF37] py-4 [&[data-state=open]]:text-[#D4AF37]">
                        {item.q}
                      </AccordionTrigger>
