      ...
      <section className="py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.6 }} className="max-w-3xl">
            ...
            <h1 className="font-['Oswald'] text-5xl sm:text-6xl font-bold uppercase mt-4 mb-6 text-white leading-[0.95]">Monthly<br /><span className="text-gold-gradient">Reviews</span></h1>
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#D4AF37] text-sm mt-4">
              Coming Soon — Subscription billing launching soon
            </div>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {tiers.map((tier, i) => (
              <div key={i} className={`p-8 ${tier.popular ? 'bg-[#0A0A0A] border-2 border-[#D4AF37]/30' : 'bg-[#0A0A0A] border border-white/5'} relative`} data-testid={`monthly-tier-${i}`}>
                ...
                <h3 className="font-['Oswald'] text-2xl font-bold uppercase text-white mb-2">{tier.name}</h3>
                ...
                <Link to="/contact" className="block text-center px-6 py-3 border border-white/20 text-white text-sm font-bold uppercase tracking-wider hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors">
                  Contact for Details
                </Link>
              </div>
            ))}
