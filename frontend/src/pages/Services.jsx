...
import { ArrowRight, Crown, Video, Users, Calendar } from 'lucide-react';
...
export default function Services() {
  ...
  useEffect(() => {
    getServices('in_person').then(r => setInPersonServices(r.data)).catch(() => {});
    getServices('virtual').then(r => setVirtualServices(r.data.filter(s => s.service_type !== 'free_tier'))).catch(() => {});
  }, []);
      ...
      <section className="py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.6 }}>
            ...
            <h1 className="font-['Oswald'] text-5xl sm:text-6xl font-bold uppercase mt-4 mb-6 text-white">Services</h1>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ visible: { transition: { staggerChildren: 0.1 } } }} className="mb-24">
            <motion.div variants={fadeUp} className="flex items-center gap-4 mb-8">
              ...
              <h2 className="font-['Oswald'] text-3xl font-bold uppercase text-white">In-Person Choreography</h2>
            </motion.div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {inPersonServices.map((s, i) => (
                <motion.div key={s.id} variants={fadeUp} className="bg-[#121212] border border-white/5 p-8 hover:border-[#D4AF37]/30 transition-colors" data-testid={`inperson-service-${i}`}>
                  ...
                  <h3 className="font-['Oswald'] text-lg font-bold uppercase text-white mb-3">{s.name}</h3>
                  <p className="text-white/40 text-sm leading-relaxed mb-6">{s.description}</p>
                  <Link to="/apply" className="inline-flex items-center gap-2 text-[#D4AF37] text-sm font-semibold uppercase tracking-wider hover:gap-3 transition-all">
                ...
                <>
                  {['Full Program Choreography', 'Choreography Camp', 'Competition Routine Polishing'].map((name, i) => (
                    <div key={i} className="bg-[#121212] border border-white/5 p-8">
                      <Crown className="w-6 h-6 text-[#D4AF37] mb-4" />
                      <h3 className="font-['Oswald'] text-lg font-bold uppercase text-white mb-3">{name}</h3>
                      ...
                      <Link to="/apply" className="inline-flex items-center gap-2 text-[#D4AF37] text-sm font-semibold uppercase tracking-wider">Apply <ArrowRight className="w-4 h-4" /></Link>
                    </div>
                  ))}
                </>
              )}
            ...
            <div className="mt-8">
              <Link to="/choreography" className="inline-flex items-center gap-2 text-white/60 text-sm hover:text-[#D4AF37] transition-colors">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
            <motion.div variants={fadeUp} className="flex items-center gap-4 mb-8">
              ...
              <h2 className="font-['Oswald'] text-3xl font-bold uppercase text-white">Virtual Services</h2>
            </motion.div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {virtualServices.map((s, i) => (
                <motion.div key={s.id} variants={fadeUp} className="bg-[#121212] border border-white/5 p-8 hover:border-[#D4AF37]/30 transition-colors" data-testid={`virtual-service-${i}`}>
                  ...
                  <Link to={s.service_type === 'review_feedback' ? '/reviews' : '/upgrades'} className="inline-flex items-center gap-2 text-[#D4AF37] text-sm font-semibold uppercase tracking-wider hover:gap-3 transition-all">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              <Link to="/reviews" className="inline-flex items-center gap-2 text-white/60 text-sm hover:text-[#D4AF37] transition-colors">Reviews & Feedback <ArrowRight className="w-4 h-4" /></Link>
              <Link to="/upgrades" className="inline-flex items-center gap-2 text-white/60 text-sm hover:text-[#D4AF37] transition-colors">Virtual Upgrades <ArrowRight className="w-4 h-4" /></Link>
              <Link to="/monthly" className="inline-flex items-center gap-2 text-white/60 text-sm hover:text-[#D4AF37] transition-colors">Monthly Reviews <ArrowRight className="w-4 h-4" /></Link>
            </div>
          </motion.div>
        </div>
      </section>
