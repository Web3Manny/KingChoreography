...
import { getServices, calculatePrice, submitSubmission, submitFreeSubmission, createPayPalOrder, capturePayPalOrder } from '@/lib/api';
...
export default function Submit() {
  ...
  const [form, setForm] = useState({
    ...
    athlete_names: [], athlete_count: 1, is_existing_client: false,
    session_length: '', consultation_addon: false, areas_of_concern: ''
  });

  useEffect(() => {
    getServices('virtual').then(r => setServices(r.data)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    if (form.service_id && form.service_type !== 'free_tier') {
      ...
      }).then(r => setPriceData(r.data)).catch(() => {});
  ...
  const handlePayAndSubmit = async () => {
    ...
    try {
      ...
      let submissionData = { ...form, athlete_names: form.athlete_names.length ? form.athlete_names : [form.full_name] };
      ...
      if (isFree) {
        ...
        const orderResp = await createPayPalOrder({ submission_id: response.data.id, amount: priceData?.total || 0 });
        await capturePayPalOrder({ order_id: orderResp.data.order_id, submission_id: response.data.id });
      }
      navigate('/submit/confirmed', { state: { submission: response.data, price: priceData } });
    } catch (err) {
      ...
      <section className="py-16 sm:py-24">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-12">
            ...
            <h1 className="font-['Oswald'] text-4xl sm:text-5xl font-bold uppercase mt-4 mb-4 text-white">Submit a<br /><span className="text-gold-gradient">Routine</span></h1>
          </div>
            ...
            <motion.div key={currentStep} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }}>
              ...
              {currentStep === 0 && (
                <div className="space-y-4" data-testid="step-service">
                  <h2 className="font-['Oswald'] text-2xl font-bold uppercase text-white mb-6">Select a Service</h2>
                  {services.map((s) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <h3 className="font-['Oswald'] text-lg font-bold uppercase text-white">{s.name}</h3>
              ...
              {currentStep === 1 && (
                <div className="space-y-6" data-testid="step-contact">
                  <h2 className="font-['Oswald'] text-2xl font-bold uppercase text-white mb-6">Contact & Program Info</h2>
              ...
              {currentStep === 2 && (
                <div className="space-y-6" data-testid="step-details">
                  <h2 className="font-['Oswald'] text-2xl font-bold uppercase text-white mb-6">Submission Details</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                  ...
                  {form.service_type === 'free_tier' && !form.is_existing_client && (
                    <div className="flex items-start gap-3 p-4 bg-red-500/10 border border-red-500/20">
                      ...
                      <p className="text-red-400 text-sm">Free submissions are available for current clients only. Please check the "Current Client" box or select a different service.</p>
                    </div>
                  )}
              ...
              {currentStep === 3 && (
                <div className="space-y-6" data-testid="step-video">
                  <h2 className="font-['Oswald'] text-2xl font-bold uppercase text-white mb-6">Upload Video</h2>
                  ...
                  <p className="text-white/30 text-xs">Note: Video upload functionality uses placeholder storage in this preview. Files will be stored in Supabase Storage in production.</p>
                </div>
              )}
              ...
              {currentStep === 4 && (
                <div className="space-y-6" data-testid="step-payment">
                  <h2 className="font-['Oswald'] text-2xl font-bold uppercase text-white mb-6">
                    {form.service_type === 'free_tier' ? 'Confirm Submission' : 'Review & Pay'}
                  </h2>

                  {/* Summary */}
                  <div className="bg-[#121212] border border-white/5 p-6 space-y-4">
                    <h3 className="font-['Oswald'] text-sm font-bold uppercase text-[#D4AF37]">Order Summary</h3>
                    ...
                    <div className="border-t border-white/10 pt-4">
                      <div className="flex justify-between items-center">
                        <span className="font-['Oswald'] text-lg font-bold uppercase text-white">Total</span>
                        <span className="font-['Oswald'] text-3xl font-bold text-[#D4AF37]">
                          {form.service_type === 'free_tier' ? 'FREE' : `$${priceData?.total?.toFixed(2) || '0.00'}`}
                        </span>
                      </div>
                      {priceData?.breakdown?.formula && (
                        <p className="text-white/30 text-xs mt-1">{priceData.breakdown.formula}</p>
                      )}
                    </div>
                  </div>
