  ...
  return (
    <div data-testid="admin-applications">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="font-['Oswald'] text-3xl font-bold uppercase text-white">Applications</h1>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                ...
                <th className="text-left p-4 text-white/40 text-xs font-bold uppercase tracking-wider hidden md:table-cell">Location</th>
                <th className="text-left p-4 text-white/40 text-xs font-bold uppercase tracking-wider hidden md:table-cell">Timeframe</th>
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.map((app) => (
                <tr key={app.id} className="hover:bg-white/5 transition-colors" data-testid={`app-row-${app.id}`}>
                  ...
                  <td className="p-4 text-white/40 text-sm hidden md:table-cell">{[app.city, app.state].filter(Boolean).join(', ') || '—'}</td>
                  <td className="p-4 text-white/40 text-sm hidden md:table-cell capitalize">{app.timeframe?.replace(/_/g, ' ') || '—'}</td>
                  <td className="p-4">
                    <span className={`px-2 py-1 text-xs font-bold uppercase rounded ${statusColors[app.status] || 'bg-gray-500/20 text-gray-400'}`}>
                      {app.status}
                    </span>
                  </td>
                  ...
                  <td className="p-4">
                    <Link to={`/admin/applications/${app.id}`} className="text-[#D4AF37] hover:text-[#FFD700]" data-testid={`app-view-${app.id}`}>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="p-8 text-center text-white/30 text-sm">{loading ? 'Loading...' : 'No applications found'}</td></tr>
              )}
            </tbody>
          </table>
        </div>
