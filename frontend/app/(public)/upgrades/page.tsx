  ...
  return (
    <div className="min-h-screen pt-20" data-testid="upgrades-page">
      <section className="py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.6 }} className="max-w-3xl">
            ...
            <h1 className="font-heading text-5xl sm:text-6xl font-bold uppercase mt-4 mb-6 text-white leading-[0.95]">Virtual Routine<br /><span className="text-gold-gradient">Upgrades</span></h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {pricingTable.map((p, i) => (
              <div key={i} className="bg-[#0A0A0A] border border-white/5 p-8 text-center hover:border-[#D4AF37]/30 transition-colors" data-testid={`pricing-tier-${i}`}>
                ...
                <p className="font-heading text-4xl font-bold text-white mb-1">{p.rate}</p>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-heading text-3xl font-bold uppercase text-white mb-12">How It Works</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {steps.map((s, i) => (
              <div key={i} className="p-6 bg-[#121212] border border-white/5">
                <span className="font-heading text-4xl font-bold text-[#D4AF37]/20">{s.step}</span>
                <h3 className="font-heading text-lg font-bold uppercase text-white mt-2 mb-2">{s.title}</h3>
      ...
      <section className="py-24 bg-[#121212] border-t border-white/5">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="font-heading text-3xl font-bold uppercase text-white mb-6">Upgrade Your Routine</h2>
          <p className="text-white/50 mb-10">Take your routine to the next level with Brian King&apos;s virtual upgrade sessions.</p>
          <Link href="/submit" className="inline-flex items-center gap-2 btn-gold px-10 py-5 text-sm" data-testid="upgrades-submit-cta">Get Started <ArrowRight className="w-4 h-4" /></Link>
        </div>
      </section>
    </div>
  )
