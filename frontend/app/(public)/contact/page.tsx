    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center" data-testid="contact-success">
        <div className="text-center px-4">
          ...
          <h1 className="font-heading text-4xl font-bold uppercase text-white mb-4">Message Sent</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.6 }}>
              ...
              <h1 className="font-heading text-5xl sm:text-6xl font-bold uppercase mt-4 mb-6 text-white">Contact</h1>
              ...
              <div className="space-y-6">
                <a href="mailto:brian@kingchoreography.com" className="flex items-center gap-4 text-white/60 hover:text-[#D4AF37] transition-colors">
                  ...
                  <div><p className="text-white text-sm font-medium">Email</p><p className="text-sm">brian@kingchoreography.com</p></div>
                </a>
                <a href="https://instagram.com/kingchoreography" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 text-white/60 hover:text-[#D4AF37] transition-colors">
                  ...
                  <div><p className="text-white text-sm font-medium">Instagram</p><p className="text-sm">@kingchoreography</p></div>
                </a>
              </div>
            </motion.div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6" data-testid="contact-form">
                <div>
                  ...
                  <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                    className="bg-black/50 border-white/10 focus:border-[#D4AF37] text-white placeholder:text-white/20 rounded-none h-12" placeholder="Your name" data-testid="contact-name" />
                </div>
                <div>
                  <label className="text-white/60 text-xs font-bold uppercase tracking-wider mb-2 block">Email *</label>
                  <Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                    className="bg-black/50 border-white/10 focus:border-[#D4AF37] text-white placeholder:text-white/20 rounded-none h-12" placeholder="your@email.com" data-testid="contact-email" />
                </div>
                <div>
                  <label className="text-white/60 text-xs font-bold uppercase tracking-wider mb-2 block">Subject</label>
                  <Input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })}
                    className="bg-black/50 border-white/10 focus:border-[#D4AF37] text-white placeholder:text-white/20 rounded-none h-12" placeholder="What's this about?" data-testid="contact-subject" />
                </div>
                <div>
                  <label className="text-white/60 text-xs font-bold uppercase tracking-wider mb-2 block">Message *</label>
                  <Textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })}
                    className="bg-black/50 border-white/10 focus:border-[#D4AF37] text-white placeholder:text-white/20 rounded-none min-h-[150px]" placeholder="Your message..." data-testid="contact-message" />
                </div>
                ...
                >{loading ? 'Sending...' : 'Send Message'} <Send className="w-4 h-4" /></button>
              </form>
            </motion.div>
          </div>
        </div>
