...
import { ArrowRight, Music, Sparkles, Target } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen pt-20" data-testid="choreography-page">
      <section className="py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.6 }} className="max-w-3xl">
            ...
            <h1 className="font-heading text-5xl sm:text-6xl font-bold uppercase mt-4 mb-6 text-white leading-[0.95]">Choreography<br /><span className="text-gold-gradient">Services</span></h1>
            ...
            <Link href="/apply" className="inline-flex items-center gap-2 btn-gold px-10 py-5 text-sm" data-testid="choreo-apply-cta">Apply for Availability <ArrowRight className="w-4 h-4" /></Link>
          </motion.div>
        </div>
      </section>
      <section className="py-24 bg-[#121212] border-y border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
            <motion.h2 variants={fadeUp} className="font-heading text-3xl font-bold uppercase text-white mb-12">What&apos;s Included</motion.h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {features.map(({ icon: Icon, title, desc }, i) => (
                <motion.div key={i} variants={fadeUp} className="bg-[#0A0A0A] border border-white/5 p-8">
                  ...
                  <h3 className="font-heading text-lg font-bold uppercase text-white mb-3">{title}</h3>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-heading text-3xl font-bold uppercase text-white mb-12">The Process</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {process.map((p, i) => (
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ delay: i * 0.1 }} className="relative p-6 bg-[#121212] border border-white/5">
                <span className="font-heading text-4xl font-bold text-[#D4AF37]/20">{p.step}</span>
                <h3 className="font-heading text-lg font-bold uppercase text-white mt-2 mb-2">{p.title}</h3>
      ...
      <section className="py-24 bg-[#121212] border-t border-white/5">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="font-heading text-3xl sm:text-4xl font-bold uppercase text-white mb-6">Ready to Get Started?</h2>
          ...
          <Link href="/apply" className="inline-flex items-center gap-2 btn-gold px-10 py-5 text-sm">Apply for Availability <ArrowRight className="w-4 h-4" /></Link>
        </div>
      </section>
    </div>
  )
