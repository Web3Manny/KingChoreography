        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-20">
          <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.15 } } }}>
            <motion.div variants={fadeUp} transition={{ duration: 0.6 }} className="mb-10">
              <img src={LOGO_URL} alt="King Choreography" className="h-40 sm:h-52 md:h-64 w-auto mx-auto drop-shadow-[0_0_30px_rgba(212,175,55,0.15)]" />
            </motion.div>
            <motion.p variants={fadeUp} transition={{ duration: 0.6 }} className="text-[#A8A9AD] text-lg sm:text-xl max-w-2xl mx-auto mb-4 leading-relaxed">
              Elite choreography for competitive cheer programs nationwide.
            </motion.p>
            <motion.p variants={fadeUp} transition={{ duration: 0.6 }} className="text-[#A8A9AD]/60 text-base max-w-xl mx-auto mb-12">
            ...
            <motion.div variants={fadeUp} transition={{ duration: 0.6 }} className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/apply" data-testid="hero-apply-btn" className="group w-full sm:w-auto inline-flex items-center justify-center gap-3 btn-gold px-10 py-5 text-sm">
                Book Brian for Your Program <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link href="/submit" data-testid="hero-submit-btn" className="group w-full sm:w-auto inline-flex items-center justify-center gap-3 px-10 py-5 border border-[#A8A9AD]/30 text-white font-bold uppercase tracking-wider text-sm hover:border-[#D4AF37] hover:text-[#D4AF37] transition-all duration-200">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
            <motion.div variants={fadeUp} transition={{ duration: 0.5 }} className="mb-16">
              ...
              <h2 className="font-heading text-4xl sm:text-5xl font-bold uppercase mt-4 text-white">Services</h2>
              <div className="divider-gold w-24 mt-4" />
            </motion.div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {services.map((service, i) => (
                <motion.div key={i} variants={fadeUp} transition={{ duration: 0.5 }}>
                  <Link href={service.link} className="group block badge-card gold-glow-hover p-8 h-full" data-testid={`service-card-${i}`}>
                    <div className="w-10 h-10 bg-gradient-to-br from-[#B8860B] to-[#D4AF37] flex items-center justify-center mb-6">
                      <span className="font-heading text-black text-lg font-bold">0{i + 1}</span>
                    </div>
                    <h3 className="font-heading text-xl font-bold uppercase text-white mb-4">{service.title}</h3>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }}>
              ...
              <h2 className="font-heading text-4xl sm:text-5xl font-bold uppercase mt-4 mb-6 text-white">The Name Behind<br />The Crown</h2>
              <div className="divider-gold w-24 mb-8" />
              <p className="text-[#A8A9AD]/70 leading-relaxed mb-4">Brian King is one of the most sought-after choreographers in competitive cheerleading. With years of experience working with elite programs across the country, he brings championship-level precision and creativity to every routine.</p>
              <p className="text-[#A8A9AD]/70 leading-relaxed mb-8">From full-program choreography to virtual routine reviews, Brian&apos;s approach combines technical expertise with an unmatched understanding of what wins on the mat.</p>
              <Link href="/about" className="inline-flex items-center gap-2 text-[#D4AF37] font-semibold uppercase tracking-wider text-sm hover:gap-3 transition-all" data-testid="about-link">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6, delay: 0.2 }}>
              <div className="aspect-[4/5] bg-[#1A1A1A] border border-[#A8A9AD]/10 relative overflow-hidden gold-glow-hover">
                ...
                <div className="absolute bottom-8 left-8 z-20">
                  <span className="font-heading text-7xl font-bold text-gold-gradient">BK</span>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
            <motion.div variants={fadeUp} transition={{ duration: 0.5 }} className="mb-16">
              ...
              <h2 className="font-heading text-4xl sm:text-5xl font-bold uppercase mt-4 text-white">What Coaches Say</h2>
              <div className="divider-gold w-24 mt-4" />
            </motion.div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {testimonials.map((t, i) => (
                <motion.div key={i} variants={fadeUp} transition={{ duration: 0.5 }} className="badge-card p-8 gold-glow-hover" data-testid={`testimonial-${i}`}>
                  <div className="flex gap-1 mb-6">
                    {[...Array(5)].map((_, j) => <Star key={j} className="w-4 h-4 fill-[#D4AF37] text-[#D4AF37]" />)}
                  </div>
                  <p className="text-[#A8A9AD]/70 text-sm leading-relaxed mb-6">&ldquo;{t.quote}&rdquo;</p>
      ...
      <section className="py-32 bg-[#121212] border-t border-[#A8A9AD]/10" data-testid="cta-section">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }}>
            ...
            <h2 className="font-heading text-4xl sm:text-5xl font-bold uppercase mb-6 text-white">Ready to Elevate<br />Your Program?</h2>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/apply" className="inline-flex items-center gap-2 btn-gold px-10 py-5 text-sm">
                Apply for Choreography <ArrowRight className="w-4 h-4" />
              </Link>
              <Link href="/submit" className="inline-flex items-center gap-2 px-10 py-5 border border-[#A8A9AD]/30 text-white font-bold uppercase tracking-wider text-sm hover:border-[#D4AF37] hover:text-[#D4AF37] transition-all">
                Submit a Routine
              </Link>
            </div>
          </motion.div>
