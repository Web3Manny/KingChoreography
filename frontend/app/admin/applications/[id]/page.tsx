...
export default function AdminApplicationDetailPage() {
  ...
  useEffect(() => {
    getAdminApplication(id as string).then(r => { setApp(r); setNotes(r.admin_notes || '') }).catch(() => router.push('/admin/applications'))
  }, [id, router])

  const updateStatus = async (status: string) => {
    setSaving(true)
    try {
      const res = await updateAdminApplication(id as string, { status })
      setApp(res)
  ...
  const saveNotes = async () => {
    setSaving(true)
    try {
      await updateAdminApplication(id as string, { admin_notes: notes })
  ...
  return (
    <div data-testid="admin-application-detail">
      <button onClick={() => router.push('/admin/applications')} className="inline-flex items-center gap-2 text-white/40 text-sm mb-6 hover:text-white transition-colors" data-testid="back-btn">
        <ArrowLeft className="w-4 h-4" /> Back to Applications
      </button>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="font-heading text-3xl font-bold uppercase text-white">{app.gym_name}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-[#121212] border border-white/10 rounded-md p-6">
            <h3 className="font-heading text-sm font-bold uppercase text-[#D4AF37] mb-4">Contact Information</h3>
          ...
          <div className="bg-[#121212] border border-white/10 rounded-md p-6">
            <h3 className="font-heading text-sm font-bold uppercase text-[#D4AF37] mb-4">Program Details</h3>
            <div className="space-y-4 text-sm">
              {app.services_interested?.length > 0 && (
                <div><span className="text-white/40 block mb-1">Services Interested</span><div className="flex flex-wrap gap-2">{app.services_interested.map((s: string, i: number) => <span key={i} className="px-2 py-1 bg-white/5 text-white text-xs">{s}</span>)}</div></div>
              )}
              {app.team_levels?.length > 0 && (
                <div><span className="text-white/40 block mb-1">Team Levels</span><div className="flex flex-wrap gap-2">{app.team_levels.map((l: string, i: number) => <span key={i} className="px-2 py-1 bg-white/5 text-white text-xs">{l}</span>)}</div></div>
              )}
            ...
            <div className="grid grid-cols-2 gap-2">
              {['new', 'contacted', 'booked', 'declined', 'archived'].map(s => (
                <button key={s} onClick={() => updateStatus(s)} disabled={saving || app.status === s}
                  ...
                  } disabled:opacity-30`} data-testid={`status-btn-${s}`}
                >{s}</button>
              ))}
            </div>
          </div>
