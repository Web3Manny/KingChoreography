...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      getAdminStats().then(r => setStats(r)),
      getRecentActivity().then(r => setActivity(r || []))
  ...
  return (
    <div data-testid="admin-dashboard">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {statCards.map((stat, i) => (
          <div key={i} className="bg-[#121212] border border-white/10 p-6 rounded-md" data-testid={`stat-card-${i}`}>
            <div className="flex items-center justify-between mb-4"><stat.icon className={`w-5 h-5 ${stat.color}`} /><TrendingUp className="w-4 h-4 text-white/20" /></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        {[
          { href: '/admin/applications', label: 'View Applications', icon: Users },
          { href: '/admin/queue', label: 'Review Queue', icon: Clock },
          { href: '/admin/submissions', label: 'All Submissions', icon: FileText },
        ].map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href} className="flex items-center justify-between p-4 bg-[#121212] border border-white/10 rounded-md hover:border-[#D4AF37]/30 transition-colors" data-testid={`quick-link-${label.toLowerCase().replace(/ /g, '-')}`}>
            <div className="flex items-center gap-3"><Icon className="w-4 h-4 text-[#D4AF37]" /><span className="text-white text-sm font-medium">{label}</span></div>
            <ArrowRight className="w-4 h-4 text-white/30" />
          </Link>
        ))}
      </div>
      <div>
        <h2 className="font-heading text-xl font-bold uppercase text-white mb-4">Recent Activity</h2>
        <div className="bg-[#121212] border border-white/10 rounded-md divide-y divide-white/5">
          {activity.length === 0 && !loading && <div className="p-8 text-center text-white/30 text-sm">No recent activity</div>}
          {activity.map((item: any, i: number) => (
            <Link key={i} href={item.type === 'application' ? `/admin/applications/${item.id}` : `/admin/submissions/${item.id}`}
              className="flex items-center justify-between p-4 hover:bg-white/5 transition-colors" data-testid={`activity-item-${i}`}
            >
              <div><p className="text-white text-sm">{item.title}</p><p className="text-white/40 text-xs">{item.subtitle}</p></div>
              <div className="flex items-center gap-3">
                <span className={`px-2 py-1 text-xs font-bold uppercase rounded ${statusColors[item.status] || 'bg-gray-500/20 text-gray-400'}`}>{item.status}</span>
